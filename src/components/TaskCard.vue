<template>
  <v-card
    class="px-4 py-3 d-block mx-auto mt-3 text-center"
    width="280px"
    :class="darker"
  >
    <v-checkbox
      ...
      class="mb-2 mt-0"
      v-model="checked"
      @click="checkCompleted($event, index)"
    ></v-checkbox>
    <v-btn class="font-weight-light text-capitalize" @click="overlay = !overlay"
      >Change</v-btn
    >
    <v-btn
      class="ml-4 font-weight-light red white--text text-capitalize"
    ...
    <v-overlay absolute :value="overlay">
      <input
        type="text"
        class="grey lighten-3 px-2 py-1 rounded d-block mb-2"
        placeholder="change task..."
        v-model="newText"
        @keydown.enter="changeTask(index)"
      />
      <v-btn
        color="success"
